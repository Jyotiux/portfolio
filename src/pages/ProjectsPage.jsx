...
import { ArrowUpRight, Boxes, Code2, Palette } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-2">
          {projects.map((project) => {
            const ProjectIcon = projectIcons[project.icon] || Code2;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  ...
                  <ProjectIcon className="h-5 w-5 text-slate-400 transition group-hover:text-teal-700" />
                </div>
                ...
                {project.tags && (
                  <div className="mt-5 flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span
                        className="rounded-full border border-teal-100 bg-white px-3 py-1 text-xs font-bold text-slate-600 transition group-hover:border-teal-200 group-hover:text-teal-800"
                        key={tag}
                      >
                        {tag}
                      </span>
                ...
                {project.links && (
                  <div className="mt-6 flex flex-wrap gap-3">
                    {project.links.map((link) => (
                      <a
                        className="inline-flex items-center justify-center gap-2 rounded-full bg-slate-950 px-4 py-2.5 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-teal-700"
                        href={link.href}
                        key={link.label}
                        rel="noreferrer"
                        target="_blank"
